...
웹 문서의 한·영 지원 범위 비교표에서 **최근 한 달 안에 처음 출시된 기능**을
`🆕` 로 표시한다.
...
- **출시일은 스크립트가 보유** — `tools/update_new_markers.py` 의 `FEATURE_DATES`
  맵(명령어 → 날짜)이 단일 소스. 지원 범위 표에는 날짜 주석을 넣지 않는다
  (표가 깨지거나 지저분해지지 않도록).
